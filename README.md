...
Redundant predictors are removed using an outcome-independent, iterative VIF threshold of 5. The highest eligible VIF across the three positions determines the next removal; VIFs are recalculated after each removal. Height, age, and applicable percentage-availability controls are protected. Selection uses a shared retained set within each model type so position comparisons use the same adjustment set. Predictive validation repeats selection inside the training folds. The final ridge and OLS sets can differ because they use different missingness handling and samples. Excluded inputs do not enter the learned signal, its contributions, or its input-coverage denominator; their original scouting statistics remain available.
...
**Full statistical details** starts collapsed. It retains each fitted position/statistic association, explicit units such as one inch or five percentage points, pointwise 95% intervals, raw and Holm-adjusted p-values, selection diagnostics, sensitivity checks, and technical CSV/JSON downloads. The primary adjustment retains the original 36 candidate association slots at a fixed 0.05 threshold, even after redundant inputs are removed. Direct tests compare retained position slopes on the same units and adjustment set. Guest access follows the existing modeling gate.
