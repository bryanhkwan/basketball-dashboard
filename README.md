...
Retained college inputs are standardized within their league and position. NBA age and salary levels are not transferred. The existing college pay anchors set the dollar scale; conference, translation, and scouting adjustments remain separate assumptions. No extra minutes multiplier is applied. Production and fit scores remain independent of the learned valuation. These are experimental estimates, not reported NCAA compensation, and the men's NBA sample does not validate WBB pay.

Redundant predictors are removed using an outcome-independent, iterative VIF threshold of 5. Selection runs independently within each position: that position's highest eligible VIF determines its next removal, and its VIFs are recalculated after every removal. An overlap in one position cannot remove a predictor from another. Height, age, and applicable percentage-availability controls are protected. Guards, Wings, and Bigs therefore have their own retained inputs as well as their own fitted coefficients. Predictive validation repeats selection inside the training folds. The final ridge and OLS sets can also differ within a position because they use different missingness handling and samples. Prediction retains 10/8/9 basketball inputs for Guards/Wings/Bigs; OLS retains 9/8/9. Rebounds remain for Guards and Bigs. Turnovers remain only in the Guards prediction model and have no primary OLS p-value. Excluded inputs do not enter the learned signal, its contributions, or its input-coverage denominator; their original scouting statistics remain available.
...
**Players → NBA salary model → Coach summary** shows one comparison table: the union of retained basketball inputs in rows, with the three independently selected Guards, Wings, and Bigs regressions side by side. Same row labels do not imply equal weights or priorities. Each cell separates the actual ridge valuation weight from the OLS coefficient, its pointwise 95% interval, raw p-value, and Holm-adjusted p-value. OLS coefficients and intervals use the stated natural increment; ridge weights use one standard deviation. P-values apply only to the OLS coefficients. Inputs omitted from one model are labeled **Excluded**, with blank CSV numeric fields rather than invented zero coefficients.

**Download 1-page table (PDF)** provides a plain-language version: the estimated percentage higher or lower NBA salary for each stated increase, a 95% range, and Supported/Uncertain labels based on Holm-adjusted p-values. These percentages convert the OLS estimates using 100 * (exp(beta) - 1); they are not ridge weights, percentages of player value, or promised raises. Excluded OLS inputs have no salary percentage or p-value. **Download table CSV** exports the retained-input comparison at full precision, without additional sensitivity rows. The **View coach summary** shortcut at the top of a player's Overview opens the table and selects that player's position for the full statistical details.

**Full statistical details** starts collapsed. It retains each fitted position/statistic association, explicit units such as one inch or five percentage points, pointwise 95% intervals, raw and Holm-adjusted p-values, selection diagnostics, sensitivity checks, and technical CSV/JSON downloads. The primary adjustment retains the original 36 candidate association slots at a fixed 0.05 threshold, even after redundant inputs are removed. Direct tests use a separate common-input analysis with the same units and adjustment set across positions. They do not subtract the primary coefficients from models with different input sets, and they do not determine player valuations. Its input list and source fits are exported explicitly. Guest access follows the existing modeling gate.

This analysis uses OLS on recorded log salary with HC3 robust uncertainty, retained basketball inputs, age, and applicable three-point/free-throw percentage-availability indicators. Observed percentages are centered before creating their unavailable placeholders, avoiding correlation caused only by coding. The primary sample retains traditional nonshooting centers and excludes one one-game guard with unavailable effective field-goal percentage. Accuracy slopes describe players with usable recorded percentages. The [analysis specification](docs/nba-salary-evidence-method.md) explains sample selection, testing families, diagnostics, and limitations.

**Prediction weights** provides the ridge model's stability and validation details; player profiles show individual contributions to that prediction model. Redundancy selection is based on predictor overlap, never coefficient signs or p-values. A positive turnover weight in the earlier model was a salary prediction pattern, not evidence that losing possessions helps winning. Lower VIF does not resolve omitted-variable bias or make the coefficients causal. Neither analysis validates NCAA compensation.
...
The handout is a static PDF generated from both the ridge and OLS snapshots, with both source hashes, the source coefficient matrix, converted percentage estimates and intervals, and the PDF hash recorded in `output/pdf/nba-salary-coach-brief.meta.json`. Regenerate it after either model changes, review `tmp_coach_brief/preview.png`, and publish the PDF and metadata together. The builder verifies consistent seasons, stat increments, and a single printed page. These optional offline dependencies add no frontend build step:
